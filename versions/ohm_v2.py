...
def crear_diagrama_fasorial_trifasico(voltajes, angulos):
    ...
    for fase, voltaje, angulo in zip(['R', 'S', 'T'], voltajes, angulos):
        ...
        if angulo != 0:  # Solo agregar arco si hay ángulo
...
def main():
    st.set_page_config(page_title="Calculadora Ley de Ohm Avanzada", layout="wide")
    st.title('🔌 Calculadora de Ley de Ohm - Avanzada')
    st.markdown("### Sistema completo para análisis de circuitos eléctricos DC, AC y trifásicos")
    ...
    with tab1:
        ...
        if tipo_circuito == "Trifásico":
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.subheader("⚙️ Parámetros por Fase")
                mostrar_resultados([
